...
"NA" stand for the missing data of attractions 
...
**Write your answer here**
The sum of data before median is much larger than that after median
...
```{r Ex_l}
visitors_all<-visitors_with_nations %>% 
  group_by(nation) %>%
  count()
```
...
visitors_free<-visitors_with_nations %>% 
  filter(admission=="Free")%>%
  group_by(nation)%>%
  count()
Total<-merge(visitors_free,visitors_all,by="nation")
Total%>%
...
na.omit(visitors)%>%
  ...
  mutate(percentage=(n_2022-n_2021)/n_2021*100,sf=2)%>%
  arrange(percentage)
